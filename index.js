...
const typo = artists.splice(8, 1, { ...artists[8], name: "Vincent Van Gogh" });
...
function get20s(arr) {
	...
	for (let i = 0; i < arr.length; i++) {
		const currElemYears = arr[i].years;
		const birth = Number(currElemYears.slice(0, 4));
		const death = Number(currElemYears.slice(-4));
